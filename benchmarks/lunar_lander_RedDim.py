...
#         leg = 1 if leg_1 != 0 or leg_2 != 0 else 0
#         return np.array([x, y, vx, vy, angle, angular_velocity, leg])
...
#         # self.state[7] = 0
#         self.step_counter = 0
#         self.state = self.reduce_state(self.state)
#         print(f"Initial state: {self.state}")
#         return self.state

#     def has_landed(self) -> bool:
#         _, y, vx, vy, angle, _, leg = self.state
#         return y <= 0.1 and leg and abs(vx) < 0.05 and abs(vy) < 0.05 and abs(angle) < 0.1

#     def has_crashed(self) -> bool:
#         _, y, vx, vy, angle, _, _ = self.state
#         return y <= 0.1 and (abs(vx) > 0.5 or abs(vy) > 0.5 or abs(angle) > 0.5)
    
#     def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[Any, Any]]:
#         x, y, vx, vy, angle, angular_velocity, leg = self.state  # Unpack reduced state
...
#         leg_1 = leg_2 = 1 if leg != 0 else 0  # Convert single leg state back to leg_1 and leg_2
#         self.state = np.array([x, y, vx, vy, angle, angular_velocity, leg_1, leg_2])
#         self.state = self.reduce_state(self.state)  # Update self.state to reduced state
...
#         return self.state, reward, done, {}  # Return the reduced state

#     def predict_done(self, state: np.ndarray) -> bool:
#         x, y, vx, vy, angle, _, leg = state
#         landed = y <= 0.1 and leg and abs(vx) < 0.05 and abs(vy) < 0.05 and abs(angle) < 0.1
